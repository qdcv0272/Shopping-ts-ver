...
export function validateName(value: string): ValidationResult {
  ...
  if (value.length < 2)
    return { ok: false, message: "이름은 2자 이상 입력해주세요." };
...
export function validatePhone(value: string): ValidationResult {
  ...
  if (digits.length < 10 || digits.length > 11)
    return { ok: false, message: "휴대폰 번호는 10~11자리여야 합니다." };
...
export function validatePassword(value: string): ValidationResult {
  ...
  if (!/[A-Z]/.test(value))
    return { ok: false, message: "대문자를 최소 1자 포함하세요." };
  if (!/[a-z]/.test(value))
    return { ok: false, message: "소문자를 최소 1자 포함하세요." };
  if (!/[^A-Za-z0-9]/.test(value))
    return { ok: false, message: "특수문자를 최소 1자 포함하세요." };
  return { ok: true };
}
...
export function validateUsername(value: string): ValidationResult {
  ...
  if (!/^[a-zA-Z0-9]+$/.test(value))
    return { ok: false, message: "영문과 숫자만 사용할 수 있습니다." };
...
export function loadUsers(): StoredUser[] {
  ...
  parsed.forEach((user) => {
    if (
      ...
      const legacyRoad =
        typeof user.roadAddress === "string" ? user.roadAddress : undefined;
      const legacyDetail =
